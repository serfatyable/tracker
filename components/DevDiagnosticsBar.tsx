		...
		<div className="fixed bottom-2 left-2 z-50">
			...
			{open ? (
				<div className="mt-2 w-[360px] rounded bg-white p-3 text-xs shadow-lg ring-1 ring-gray-200">
					...
					<div className="mb-1">Firebase configured: <span className={status.ok ? 'text-green-600' : 'text-red-600'}>{String(status.ok)}</span></div>
					...
					<div className="text-gray-600">Route: {typeof window !== 'undefined' ? window.location.pathname : '/'}</div>
				</div>
			) : null}
		</div>
	);
