    ...
    <div className="flex gap-3 overflow-x-auto">
                ...
                >
                  <div className="text-sm font-medium flex items-center justify-between">
                    ...
                    <span className="text-xs opacity-70">
                      {(() => {
                        const y = yearsSince((resident as any).residencyStartDate);
                        return y == null ? 'PGY -' : `PGY ${Math.max(1, y + 1)}`;
                      })()}
                    </span>
                  </div>
                  <div className="mt-1 flex items-center gap-1 flex-wrap">
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() =>
                              setUnassignState({ residentId: resident.uid, tutorId: tid })
                            }
                          >
                            ×
                          </button>
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() => setTutorAssignState({ residentId: resident.uid })}
                    >
                      Assign tutor
                    </Button>
                    <Button
                      ...
                      className="btn-levitate"
                      variant="outline"
                      onClick={() => setMoveState({ residentId: resident.uid, rotationId: rot.id })}
                    >
                      Move
                    </Button>
                  </div>
      ...
      <Dialog open={!!moveState} onClose={() => setMoveState(null)}>
        <div className="p-3 space-y-2">
          ...
          <DialogFooter>
            <Button variant="secondary" onClick={() => setMoveState(null)}>
              Cancel
            </Button>
            <Button onClick={onMoveConfirm} disabled={!moveState?.rotationId}>
              Confirm
            </Button>
          </DialogFooter>
        </div>
      ...
      <Dialog open={!!tutorAssignState} onClose={() => setTutorAssignState(null)}>
        <div className="p-3 space-y-2">
          ...
          <DialogFooter>
            <Button variant="secondary" onClick={() => setTutorAssignState(null)}>
              Close
            </Button>
          </DialogFooter>
        </div>
      ...
      <Dialog open={!!unassignState} onClose={() => setUnassignState(null)}>
        <div className="p-3 space-y-2">
          ...
          <DialogFooter>
            <Button variant="secondary" onClick={() => setUnassignState(null)}>
              Cancel
            </Button>
            <Button
              onClick={async () => {
                if (!unassignState) return;
                await onUnassignTutor(unassignState.residentId, unassignState.tutorId);
