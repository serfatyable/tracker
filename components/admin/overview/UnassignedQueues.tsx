...
export default function UnassignedQueues({ assignments, residents, rotations }: Props) {
    const { unassignedResidents } = useMemo(() => {
        ...
        return { unassignedResidents: residents.filter((r) => r.role === 'resident' && !assigned.has(r.uid)) };
        ...
        <div className="glass-card p-3">
            ...
            <div className="space-y-2">
                {unassignedResidents.length === 0 ? (
                    ...
                    <div key={r.uid} className="flex items-center justify-between border rounded p-2">
                        <div className="text-sm">{r.fullName || r.uid}</div>
                        <div className="flex items-center gap-2">
                            {rotations.map((rot) => (
                                <Button key={rot.id} size="sm" onClick={() => assignResidentToRotation(r.uid, rot.id)}>{rot.name}</Button>
                            ))}
                        </div>
                    </div>
                ))}
