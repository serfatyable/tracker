...
export default function TemplateImportDialog({ open, onClose, onImported }: Props) {
    ...
    async function doImport() {
        ...
        const res = await importRotationFromCsv({ mode, csvText, rotationMeta: mode==='create' ? { name: `Imported ${new Date().toISOString().slice(0,10)}`, startDate: (await import('firebase/firestore')).Timestamp.fromDate(new Date()), endDate: (await import('firebase/firestore')).Timestamp.fromDate(new Date()), status: 'active' } : undefined });
    ...
    return (
        <Dialog open={open} onClose={onClose}>
            ...
            <div className="space-y-3">
                <div>
                    <a className="text-sm text-teal-700 underline" href="/api/templates/rotation.csv">{t('ui.downloadTemplate')}</a>
                </div>
                <div>
