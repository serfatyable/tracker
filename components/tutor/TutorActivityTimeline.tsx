      ...
      >
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-center text-sm text-gray-600 dark:text-gray-300">
            {t('tutor.timeline.noActivity', { defaultValue: 'No recent activity yet' })}
          </p>
        </div>
      </Card>
    );
      ...
      <div className="relative">
        ...
        <div className="space-y-6">
          {activities.map((activity, index) => {
            const typeStyle = activity.type ? typeStyles[activity.type] : undefined;
            const Icon = activity.icon || typeStyle?.icon || ClockIcon;
            const iconColor =
              activity.color || typeStyle?.color || 'text-violet-600 dark:text-violet-400';
            const iconBg = activity.bgColor || typeStyle?.bgColor || 'bg-violet-500/10';
            ...
            return (
              <div key={activity.id} className="relative flex gap-4 pb-2">
                {/* Icon */}
                <div
                  className={`relative z-10 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full ${iconBg} ring-4 ring-white dark:ring-slate-900`}
                >
