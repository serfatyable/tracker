        ...
        {items.map((c) => {
          ...
          return (
            <li key={c.id || c.dateKey + c.title} className="p-2 flex items-center justify-between">
              ...
              {c.link ? (
                <Link href={c.link} className="text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">
                  {t('ui.open')}
                </Link>
              ) : null}
            </li>
    ...
    <div>
      ...
      <div className="mx-auto max-w-5xl p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          <SectionBox title={t('morningMeetings.today')}>
            {renderList(today)}
          </SectionBox>
          <SectionBox title={t('morningMeetings.tomorrow')}>
            {renderList(tomorrow)}
          </SectionBox>
          <SectionBox title={t('morningMeetings.next7')}>
            {renderList(next7)}
          </SectionBox>
        </div>
        <div className="glass-card p-4">
                ...
                <div className="space-y-1">
                  {(monthByDay[d] || []).map((c) => (
                    <div key={c.id || c.title} className="truncate">
                      {c.link ? (
                        <Link href={c.link} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                          {c.title}
                        </Link>
                      ) : (
                        <span>{c.title}</span>
