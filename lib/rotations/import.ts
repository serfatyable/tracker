...
export function normalizeParsedRows(rows: ParsedRow[]): { leaves: NormalizedLeaf[]; errors: string[] } {
    const errors: string[] = [];
    const leaves: NormalizedLeaf[] = [];
    const allowedCats = new Set(['Knowledge', 'Skills', 'Guidance']);
    rows.forEach((r, idx) => {
        const rowNum = idx + 2; // header is line 1
        if (!allowedCats.has(r.Category)) errors.push(`Row ${rowNum}: invalid Category '${r.Category}'`);
